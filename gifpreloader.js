...
(function($, window, document, undefined) {
    ...
    Plugin.prototype = {
        ...
        init: function() {
            ...
            // and not inline data
            if (!this.element.complete && this.element.readyState !== 'complete' &&
                this.element.readyState !== 4 && (this.source.substr(0, 4) === 'http' ||
        ...
        imageLoaded: function() {
            if (!this.hasImageLoaded) {
                // if the image has not been loaded before, reset the src attributes to re-play the
                // animation
                this.hasImageLoaded = true;
                this.$image.attr('src', this.source);
            } else {
                // if the image has been loaded before then remove the placeholder and unwrap the
                // image element as the animation has been restarted
